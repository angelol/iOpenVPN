...
io_wait_dowork (struct context *c, const unsigned int flags)
{
  ...
  if (c->c2.event_set_status & ES_ERROR)
	...
	fprintf(stderr, "I/O WAIT status=0x%04x", c->c2.event_set_status);
...
{
	fprintf(stderr, "process_io called\n");
  ...
  if (status & SOCKET_WRITE)
    {
			fprintf(stderr, "process_outgoing_link\n");
      process_outgoing_link (c);
    }
  /* TUN device ready to accept write */
  else if (status & TUN_WRITE)
    {
			fprintf(stderr,"process_outgoing_tun\n");
      process_outgoing_tun (c);
    }
  /* Incoming data on TCP/UDP port */
  else if (status & SOCKET_READ)
    {
			fprintf(stderr, "Incoming data on TCP/UDP port\n");
  ...
  /* Incoming data on TUN device */
    ...
    {
			fprintf(stderr, "Incoming data on TUN device\n");
      read_incoming_tun (c);
      if (!IS_SIG (c))
